...
class QsCoinMkScraper(object):
    ...
    def refresh_kline_data(self, refresh_cointype="bitcoin"):
        ...
        """
        # 获取要更新的对应数据文件
        file_list = os.listdir(ABuEnv.g_project_cache_dir)
        found = False 
        target_file = ""
        for filename in file_list:
            if refresh_cointype in filename:
                target_file = os.path.join(ABuEnv.g_project_cache_dir,filename)
                found = True
                break
        if not found:
    ...
    def parse_page(self,url):
        ...
        for tr in all_tr[::-1]:
            ...
            td_date = ABuDateUtil.date_str_to_int(td_date_index)
    ...
    def save_cache(self, datadf):
        ...
        fullpath = os.path.join(ABuEnv.g_project_cache_dir, filename)
